...
@Component({
  ...
  templateUrl: './vista-dda.component.html',
  styleUrl: './vista-dda.component.css'
})
export class VistaDDAComponent {
  ...
  ocultarAprovacion()
  {
    const navElement = document.getElementById('divAprovacion');
    if (navElement) {
      navElement.style.display = 'none';
    }
  }
  ...
  boletasAprobadas(){
    this.BoletaAprobadaArray = this.BoletaArray.filter(boleta => boleta.aprobacion1 === 1);
  };

  boletasFullAprobadas(){
    this.BoletaFullArray = this.BoletaArray.filter(boleta => boleta.aprobacion2 === 1);
  };

  ocultarReporte()
  {
    const navElement = document.getElementById('divFullAprobado');
    if (navElement) {
      navElement.style.display = 'none';
    }
  }
